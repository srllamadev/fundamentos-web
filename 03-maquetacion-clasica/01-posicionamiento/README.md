...
### Ejercicio 1: Badge de notificacion
Crea un icono de campana (🔔) con un badge rojo en la esquina superior derecha que muestre "3". Usa `position: relative` en el contenedor y `position: absolute` en el badge.

```
┌────────────┐
│     🔔  3  │  ← badge absolute en esquina sup der
└────────────┘
```

### Ejercicio 2: Navbar sticky
Crea una barra de navegacion que se quede pegada arriba cuando el usuario haga scroll hacia abajo. Incluye el nombre "TextilPro S.A." y enlaces.

### Ejercicio 3: Modal de alerta
Crea un modal centrado en pantalla usando `position: fixed` con un overlay oscuro de fondo. El modal debe mostrar una alerta de produccion para TextilPro.

### Ejercicio 4: Boton "Volver arriba"
Crea un boton flotante en la esquina inferior derecha que aparezca cuando el usuario haga scroll. Usa `position: fixed`.

### Ejercicio 5: Tabla con header sticky
Crea una tabla con muchas filas donde la primera fila (encabezados) se quede pegada arriba al hacer scroll. Usa `position: sticky` en el `th`.

---

## Conexiones

- Siguiente tema: [Float y Clear](../02-float-y-clear/)
- Siguiente seccion: [Maquetacion Moderna - Flexbox](../../04-maquetacion-moderna/01-flexbox/)
